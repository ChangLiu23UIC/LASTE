...
#  ** BIGGER variant: 2x axis fonts, larger sample labels, larger markers **
#
#  Usage examples:
#    python src/umap_tuning_large.py
#    python src/umap_tuning_large.py --n_neighbors 5 --min_dist 0.1
#    python src/umap_tuning_large.py --metric cosine --leiden_resolution 0.5
#    python src/umap_tuning_large.py --n_neighbors 3 --min_dist 0.1 --repulsion_strength 2.0 --spread 1.5
#    python src/umap_tuning_large.py --n_neighbors 3 --min_dist 0.05 --out results/oe/figures/UMAP_test
#
...
_DEFAULT_OUT = str(RESULTS_DIR / "oe" / "figures" / "UMAP_OE_large")
...
_p.add_argument("--label_offset",       type=float, default=0.5,           help="Label nudge in data units (default 0.5)")
_p.add_argument("--out",                type=str,   default=_DEFAULT_OUT,  help="Output path without extension (default results/oe/figures/UMAP_OE_large)")
_args = _p.parse_args()
...
for i, s in enumerate(samples):
    ...
    if polarity == "pos":
        ax.scatter(embedding[i,0], embedding[i,1],
                   ...
                   marker=marker, s=320, linewidth=0.8, alpha=0.92, zorder=3)
    else:
        ax.scatter(embedding[i,0], embedding[i,1],
                   facecolors="none", edgecolors=color,
                   marker=marker, s=320, linewidth=2.2, alpha=0.92, zorder=3)
...
_LABEL_SIDE_OVERRIDES = {
    ...
    "OE1_PB_pos_R2": "left",
    "OE1_PB_neg_R1": "right",
    "OE1_PB_neg_R3": "right",   # top-right of point
    "OE2_PB_neg_R2": "right",
    "OE2_PB_pos_R2": "right",
    "OE1_NPB_neg_R2": "right",
}
...
_LABEL_SAME_Y = {"OE1_PB_pos_R2", "OE1_PB_neg_R3"}

# Labels pinned at their computed position — excluded from adjust_text
_LABEL_PINNED = {"OE2_PB_neg_R3", "OE1_NPB_neg_R2", "OE1_PB_pos_R3", "OE2_PB_pos_R2"}
...
for i, s in enumerate(samples):
    ...
    t = ax.text(x0, y0, s, fontsize=15, ha=ha, va=va,
                fontfamily="Arial", color=label_color, fontweight="bold",
                clip_on=False)
    if s not in _LABEL_PINNED:
        texts.append(t)
...
ax.set_xlabel("UMAP-1", fontsize=36, fontweight="bold", fontfamily="Arial")
ax.set_ylabel("UMAP-2", fontsize=36, fontweight="bold", fontfamily="Arial")
ax.set_title("OE Proteomics - UMAP", fontsize=40, fontweight="bold", fontfamily="Arial")
